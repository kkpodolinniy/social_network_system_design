...
### Оценка дисков
Поскольку самое дорогостоящее это посты, я выбрал 32TB диски SSD на которые ориентировался по всем остальным просчетам
##### Посты
...
Берем максимальные 32TB диски, понятно, что в реальности это будет очень дорого. Также брал в разрезе разных подсистем разные диски, изолированно, просто чтобы посчитать
**Объем**:  17430 / 32 = 545 дисков

**Операции ввода-вывода в секунду**: 1526 RPS/ 1000 = 1.53
...
**Объем**: 250.71 / 32TB = 7.83 (8дисков)

**Операции ввода-вывода в секунду**:  5600 / 1000 = 5.6 (6 дисков)

**Пропускная способность**:  7.95 MB/сек / 500 = 0.01
...
**Объем**:  16.39 / 32TB - 0.51

**Операции ввода-вывода в секунду**: 695 / 1000 = 0.69

**Пропускная способность**:  1.56 MB/сек / 500 = 0.003
...
**Операции ввода-вывода в секунду**: 416/ 1000 - 0.41

**Пропускная способность**:  9,41мб/с / 500  = 0.02

#### Считаем общее (самая большая величина по каждой подсистемы)
- 545 дисков для постов
- 8 дисков для лайков
- 0.7 дисков для комментариев
- 0.4 диска для пользователей
  ИТОГО: 554 диска + 20 процентов запаса = 665 диска SSD объемом 32TB